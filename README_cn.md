...
0.4.1 版本增加可编辑漫画工作区边界：

- 将机器 OCR 与人工区域修订分开保存，支持多边形、文本、语言、方向、阅读顺序、拆分、合并、删除、恢复、批准和撤回；
- OCR 重跑保留人工修订，并将受影响的译文标记为过时，不覆盖已审核数据；
- 通过现有命名供应商适配器显式翻译单个区域，在不含凭据的 sidecar 中保存机器译文、原文修订版本、审核状态和来源；
- 提供不扁平化、不修改导入页面的译文覆盖层和可访问区域编辑器基础组件。
- 提供本地 OCR 模型包安装、SHA-256 校验、延迟运行时构建和有界基准证据生成；当前不封装 OCR 引擎或模型权重。

下列主要功能尚待开发：

- 生产 OCR 运行时和模型选择，必须先通过本地许可证、校验和、平台基准及资源占用门槛；
- 擦字修复、inpainting、译文嵌字和独立译文导出；
