...
export const useCustomTextureStore = create<TextureStoreState>((set, get) => ({
  ...
  addTexture: (path, options) => {
    ...
    if (existingTexture) {
      if (existingTexture.blobUrl) {
        URL.revokeObjectURL(existingTexture.blobUrl);
      }
      const updatedTexture: CustomTexture = {
        ...existingTexture,
        ...texture,
  ...
  removeTexture: (id) => {
    ...
    set((state) => ({
      textures: state.textures.filter((entry) => entry.id !== id),
    }));
    return true;
  },
  ...
  isTextureLoaded: (textureId) => {
    const texture = get().getTexture(textureId);
    return texture?.loaded === true && !texture.error;
  },
